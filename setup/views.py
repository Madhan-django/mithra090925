...
from .forms import add_Acad_Form,set_current_yr,add_class,add_section,add_subjects
...
def academicyear(request):
    ...
    return render(request,'setup/academic-year.html',context={'data':data,'skool':sdata})
...
def add_acad_yr(request):
    ...
    return render(request,'setup/add_acad_yr.html',context={'form':form})
...
def current_acad_yr(request):
    ...
    return render(request,'setup/set_current.html',context={'adata':adata,'skool':sdata,'ydata':ydata,'sdata':sdata})
...
def update_acad_yr(request,acadyr_id):
    ...
    year = academicyr.objects.filter(school_name=skool)
    ...
    return render(request,'setup/update_current.html',context={'form':form,'year':year})
...
def delsec(request,sec_id):
    ...
    data.delete()
    return HttpResponseRedirect('listsec')
...
def subjects_list(request):
    ...
    return render(request,'setup/subjects.html',context={'skool':sdata,'year':year,'data':data})
...
def addsubject(request):
    ...
    return render(request, 'setup/addsubjects.html', context={'form': form,'skool':sdata,'year':year})
...
def edit_academic_yr(request,acadyr_id):
    data = academicyr.objects.get(pk=acadyr_id)
    form = add_Acad_Form(request.POST or None,instance=data)
    if form.is_valid():
        form.save()
        messages.success(request, 'Session Updated Successfully')
        return redirect('academicyr')
    return render(request, 'setup/update_academicyr.html', context={'form': form})
...
def add_admin(request):
    ...
    if request.method == 'POST':
        ...
        date_str1 = request.POST['dob_day']
        date_str2 = request.POST['dob_month']
        date_str3 = request.POST['dob_year']
        ...
        dtmn = date_str2
        dtday = date_str3
        usernm = name_str + name_str2 + dtday + dtmn + dtyr
