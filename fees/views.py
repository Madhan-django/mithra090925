...
from .forms import fee_addform,addindfeeform,addbulkfeeform,addfeerecieptform
...
def fee_add(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            redirect('fee_details')
        else:
            messages.info(request,'Form Invalid')

    form = fee_addform(initial=initial_data)
    return render(request,'fee/addfee.html',context={'form':form,'options':cdata,'skool':sdata,'year':year})
...
def addfeeind(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            tmp.invoice_no=fees.objects.filter(fees_school=sch_id).count() + 1
            tmp.save()
    ...
    return render(request,'fee/addindfee.html',context={'form':form,'data':data,'cdata':cdata,'skool':sdata,'form1':form1})
...
def addbulkfee(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            for student in stud:
                inv_no = fees.objects.filter(fees_school=sch_id).count()+1
                addindfee.objects.create(fee_cat=fee_cat,class_name=fee_class,stud_name=student,concession=0,status='Unpaid',invoice_no=inv_no,due_amt=fee_cat.fee_amount)
        messages.success(request, 'Fee has been Generated Successfully.')
        return redirect('invoices')
    form = addbulkfeeform()
    return render(request,'fee/bulkfee.html',context={'form':form,'skool':sdata})
...
def updatefee_cat(request,fee_cat_id):
    ...
    form =fee_addform(request.POST or None,instance=data)
    if form.is_valid():
        form.save()
        messages.success(request,'Record Updated Successfully')
        return redirect('fee_details')
    return render(request, 'fee/updatefee.html',context={'form': form,'skool':sdata})
...
def updateindfee(request,feeind_id):
    ...
    form = addindfeeform(request.POST or None,instance=data)
    if form.is_valid():
        form.save()
        messages.success(request, 'Record Updated Successfully')
        return redirect('invoices')
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                if paid_amt1 == tot1:
                    ...
                    recindfee.due_amt=0
                    recindfee.save(update_fields=["status","due_amt"])
                    form.save()
                    rec_data = fee_reciept.objects.get(reciept_no=rec_no)
                    return render(request,'fee/reciept_show.html',context={'data':rec_data,'sch_name':sdata})
                elif paid_amt1<tot1:
                    recindfee.status= 'Partially Paid'
                    recindfee.due_amt = recindfee.due_amt - paid_amt1
                    recindfee.save(update_fields=["status","due_amt"])
                    form.save()
                    messages.success(request, 'Fee Partially Paid')
                    rec_data = fee_reciept.objects.get(reciept_no=rec_no)
                    return render(request, 'fee/reciept_show.html', context={'data': rec_data, 'sch_name': sdata})
                else:
...
def invoice_search(request):
    ...
    if Searchby == 'studname':
        ...
        data = students.objects.filter(stud_name__admn_no=Searched)
    elif Searchby == 'CClass':
        data = students.objects.filter(class_name=Searched)
    elif Searchby == 'status':
        data = students.objects.filter(status=Searched)
    else:
